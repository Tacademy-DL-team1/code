# 건설 현장 안전고리 체결 판별 시스템 — 코드

건설 현장 영상에서 작업자 주변의 안전고리를 찾고, 구조물 체결 여부를 분류하는 딥러닝 프로젝트의 코드 저장소입니다. 최종 객체 탐지·인스턴스 분할 모델인 RF-DETR로 안전고리 후보 영역을 검출하고, EfficientNet 또는 ViT 분류기로 `connected`와 `unconnected` 상태를 판별합니다.

YOLO 계열 모델은 탐지 성능을 비교하고 최종 모델을 선정하기 위한 실험에 사용했으며, 최종 통합 파이프라인에는 RF-DETR을 적용했습니다.
...
2. RF-DETR로 안전고리 후보 영역을 검출·분할합니다.
3. EfficientNet 또는 ViT로 체결 여부를 분류합니다.
4. ByteTrack과 상태 판정 로직을 결합해 영상 단위 결과를 생성합니다.

## 모델 구성

- 최종 객체 탐지·분할: RF-DETR Segmentation Large
- 비교 실험: YOLOv5, YOLOv8, YOLO11, YOLO12, RT-DETR
- 안전고리 체결 상태 분류: EfficientNet, ViT
- 영상 추적 및 통합: ByteTrack 기반 파이프라인
